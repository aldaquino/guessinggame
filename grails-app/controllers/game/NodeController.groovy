...
class NodeController {
    ...
    def getFirstQuestion() {
    	println("getFirstQuestion")
        //get root node associated with first question
        def rootNode = getRoot()
        //set current node in data base
        setCurrentNodeId(rootNode)
        //get the current node id and fetche's the node it self
        def curNodeId = getCurrentNodeId()
        rootNode = Node.get(curNodeId)
    ...
    def showLeafNodeMessage(boolean userButtonChoice) {
        ...
        if (userButtonChoice) {
            ...
            render("See, I knew what you where thinking!")
        }
    }
